...
"""
Functions for downloading pre-trained Jodi models
"""
import os
...
from tools.hf_utils import hf_download_or_fpath
...
def find_model(model_name):
    ...
    """
    # Load a custom Jodi checkpoint:
    model_name = hf_download_or_fpath(model_name)
    assert os.path.isfile(model_name), f"Could not find Jodi checkpoint at {model_name}"
    print(colored(f"[Jodi] Loading model from {model_name}", attrs=["bold"]))
